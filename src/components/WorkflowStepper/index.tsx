        ...
        return (
          <div key={step.id} className="flex items-center">
            <Button
              ...
              onClick={() => console.log(`Go to step ${step.id}`)}
              className={cn(
                "flex items-center gap-2 px-10 py-6 rounded-full transition-all duration-300",
                isCurrent && "bg-orange-500 text-primary-foreground shadow-soft",
                isCompleted && "bg-green-100 text-green-600",
                !isCurrent && !isCompleted && "text-muted-foreground bg-gray-100"
              )}
            >
              <span
                className={cn(
